...
- **CSS**: **Utilities-first + shadcn/ui.** UI is built on the shadcn/ui primitives in `src/components/ui/` (wrapped by the `src/ui/` barrel and themed to the OmniVoice palette), composed with Tailwind v4 utility classes mapped to the design tokens in the `@theme` / `[data-theme]` blocks of `src/index.css` — the **single token-foundation file** (`tokens.css`/`themes.css` were consolidated into it). The irreducible CSS that utilities genuinely can't express — glassmorphism/`backdrop-filter`, `@keyframes`, pseudo-elements, `:has()`, cascade overrides of unlayered rules, and styling hooks on library-generated DOM (virtualized rows, WaveSurfer) — now lives in exactly **two files**: `src/index.css` (the token foundation **plus every `@keyframes`**, all centralized so animations resolve globally) and `src/styles/residual.css` (the consolidated glass/pseudo/cascade-override stubs, loaded immediately after `index.css`). The former per-component `.css` files were folded into these two. New components should **not** spawn a `.css` file — reach for shadcn primitives + utilities first, and if a truly-irreducible rule is unavoidable, add it to `residual.css` (or a `@keyframes` to `index.css`). (The CSS→Tailwind/shadcn migration is complete; see `docs/shadcn-migration.md`.)
