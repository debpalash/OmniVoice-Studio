...
> **Codebase-grounding note (this revision):** every file/line/symbol below was verified against the working tree on `feat/stories-shared-render`. Corrections applied vs. the prior draft: MLX line numbers (`tts_backend.py:579`, `asr_backend.py:507/510`), a mis-attributed CosyVoice anchor (`:846` is **GPTSoVITS**, CosyVoice is `:682`), the **LLM registry has no local-compute backends** (so GPU-family routing is `n/a` for LLM — see §3a), two engines missing from the audit (**`engines/supertonic3/backend.py:75`** and **`engines/indextts/__init__.py` `IndexTTS2Backend`** which inherits the CPU-only ABC default despite being CUDA-class), a **third pre-existing detector** (`services/gpu_sandbox.py`) to reconcile, the precise `get_best_device()` side-effects (`_configure_rocm_if_needed`, not "ROCm GFX"), and the Pydantic `extra="allow"` nuance on the preflight schemas.
>
> **Completeness-pass note (this revision):** the synth-time gating in the prior §7 covered only `generation.py`'s `generate_speech`. The working tree has **three more synth entry points** that resolve a backend and gate on `is_available()` but not routing — the OpenAI-compat `POST /v1/audio/speech` (`openai_compat.py:252` via `_resolve_engine:142-170`, including the `tts-1`/`tts-1-hd` aliases), and the `/ws/tts` WebSocket (`tts_stream.py:87-99`). All three are now enumerated in §7 so the no-silent-fallback bar holds on **every** path that turns text into audio, not just the REST form-POST. This pass also fully specifies (a) every failure/empty/degenerate state of `detect_host_caps()` itself, (b) the cache-lifetime/refresh contract and what (if anything) invalidates it, (c) the resolver's behavior on the device families that are **not** in the `DeviceFamily` enum (DirectML, XPU edge), (d) the "no active engine / active engine unknown or uninstalled" preflight states, and (e) the frontend's empty/legacy/unknown-status rendering.
>
> **Constraints-pass note (this revision):** added the rule-by-rule **§Constraints — OmniVoice hard rules** section mapping each CLAUDE.md hard rule (cross-platform parity, local-first, backward-compatible data, CodeQL py/polynomial-redos, localization, versioning) to the concrete design decision that satisfies it, and corrected a redaction conflation: `_mask_hf_tokens` (`tts_backend.py:45`, **only** masks `hf_[A-Za-z0-9]{30,}`) is **not** the same as `core.scrub.scrub_text` (`backend/core/scrub.py:70`, masks HF + GitHub PAT/classic + OpenAI `sk-` keys + home dirs + secret-NAMED env-var values). Routing reasons are author-controlled strings (never user input), but because they can interpolate a `device_name` or an exception message, the spec now routes them through **`scrub_text`** (the stronger scrubber), not just `_mask_hf_tokens`, before serialization — see §Constraints (local-first) and §5/§7. Confirmed: 21 locale files present; version files at `0.3.6` (latest release `v0.3.5` + 1 patch, per the versioning hard rule); no `localStorage`/DB/alembic surface touched by this task.
>
> **API + data-shapes-pass note (this revision):** every new/changed wire shape, function signature, Pydantic model, WS frame, and HTTP header is now pinned exactly so a developer can implement against it without guessing. Specifically: (a) `resolve_routing` now returns a typed `RoutingResult` `TypedDict` (was a bare `-> dict`) with the exact key set and value domains; (b) `detect_host_caps()` / `refresh()` / `mlx_supported()` have full verified signatures and the `HostCaps` frozen dataclass is field-pinned with types + defaults; (c) the four new `/engines`-entry keys are enumerated with their JSON types and exact value enums for TTS / ASR / LLM / degraded / raised-availability variants; (d) the `select_engine` response gains the routing fields with exact shape, and the `unavailable`-block `HTTPException` `detail` string is pinned; (e) new Pydantic models `GpuRouting` (for `PreflightResponse.gpu_routing`) and `DeviceInfo` field additions are written out with field names/types/defaults, plus the `extra="allow"` trap is called out per-model; (f) the two synth HTTP headers (`X-OmniVoice-Routing`, `X-OmniVoice-Routing-Reason`) have pinned value domains, encoding rules, and emit-conditions; (g) the WS `routing` frame is given an exact JSON schema sitting alongside the existing `start`/`done`/error frames; (h) the dub `preflight_error` SSE channel's warning-prefix vs. blocking-`error`-event shapes are pinned; (i) a consolidated **§API / data shapes** reference now carries the canonical JSON for every endpoint, frame, and header in one place. **No DB schema and no migration are in scope — this task adds none; the "DB schema + migration" lens item is explicitly N/A and the reason is recorded in §Constraints (backward-compatible data) and §API.** Verified during this pass: `scrub_text` signature is `scrub_text(text: str | None) -> str` and **never returns `None`** (`scrub.py:70-78` — `None`→`""`), so `routing_reason` serialized through it is `str`; `select_engine` response today is exactly `{family, active, env_override}` (`engines.py:285-289`); the REST `StreamingResponse` header dict is the 6 keys at `generation.py:496-503`; the WS frames are `send_json` objects discriminated by `"type"` (`tts_stream.py:191-197,220-227`); `PreflightCheck`/`DeviceInfo` carry `extra="allow"` and `PreflightResponse` does **not** (`schemas.py:108-139`).
