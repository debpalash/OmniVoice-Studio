...
> **Codebase-grounding note (this revision):** Every file/line/symbol reference below was verified against the working tree on branch `feat/stories-shared-render`. Corrections from the prior draft are called out inline with **[grounded]** tags so a reviewer can see what moved. Notable fixes: `genCastId()` lives in `StoriesEditor.jsx` (not `utils/storyCast.js`); `uiSlice.ts` is only 124 lines so its field/setter/init line numbers were all stale; `store/index.ts` `partialize` spans `:72-114`; `AudiobookTab` reads `profiles` from a **prop**, not the store; the `Menu` component is **item-array-driven**, not children-as-items; and there is **no** `uiSlice.test.ts` to mirror.
>
> **Completeness pass (prior revision):** added a dedicated **§Edge cases, failure & empty states** section that enumerates every branch the two new actions must survive — concurrent / repeated clicks, materialize failure, offline community, `mode`-switch races, a Stories project being mid-edit when the handoff lands, profile-WS never arriving, `''`/null/stale `audiobookDefaultVoice`, the `Menu` trigger disabled state, and persistence-migration corner cases. Acceptance criteria expanded to cover them. Nothing previously specified was removed.
>
> **Project-constraints pass (prior revision):** verified every claim that touches an OmniVoice hard rule against the working tree — `store/index.ts:115` `version: 4` and the pass-through `migrate` at `:120-130`; `uiSlice.ts:16-30` `AppMode` union (no `'audiobook'`); `storiesSlice.ts:104-106` `loadProject`/`newProject` explicit-key writes; `VoiceGallery.jsx:57` `const { t } = useTranslation()` and the existing `t(...)`-with-`defaultValue` idiom; `tests/test_no_hardcoded_cjk.py:40` allowlist (`frontend/src/i18n/` prefix); `en.json:916` `gallery.use_voice`. Nothing previously specified was removed.
>
> **API + data-shapes pass (prior revision):** pinned every wire shape, store shape, and function signature this task touches so it can be implemented without guessing. Verified at the source/wire level: the backend `/use` routes return **exactly** `{"profile_id": <str>, "name": <str>}` (`backend/api/routers/archetypes.py:310`, `community.py:280`) where `profile_id = str(uuid.uuid4())[:8]` (an **8-char** hex slug, NOT a UUID), and both routes emit the realtime WS event **`"profiles"` with payload `{"action": "created", "id": <profile_id>}`** (`archetypes.py:309`, `community.py:279`) — the precise event `useAppData.js:114` is subscribed to. The TS wrappers (`useArchetypeAsProfile`, `addCommunityItem`) are both typed `Promise<{ profile_id: string; name: string }>`. The `Profile` consumer shape lives at `api/types.ts:109` (`{ id; name; kind; language_code?; … }`). New store fields are given exact TS signatures and defaults. **[grounded fix]** Community `onUse` today flashes `gallery.saved_as_profile` (`VoiceGallery.jsx:505`, key at `en.json:920`), not the failure key the prior draft implied — recorded below so the new handlers reuse the right key family. **[grounded fix]** The `gallery` i18n block is at `en.json:892-`; `use_voice` is `:916`, the new keys go in that block. Nothing previously specified was removed.
>
> **Test-plan pass (this revision):** the §Test plan was rebuilt around the **pure-slice / handler-direct** strategy that the existing frontend suite already uses, verified end-to-end against the working tree so a reviewer can copy the patterns:
> - **Harness is wire-verified.** `frontend/src/store/storiesSlice.test.ts:4-10` builds a slice with a plain `set`/`get` closure: `state = createStoriesSlice(set, get, {})`. `updaterSlice.test.ts:5-9` does the identical thing for a *different* slice — proving the pattern generalizes, so a **new** `uiSlice.test.ts` calls `createUiSlice(set, get, {})` the same way (`createUiSlice` is the `StateCreator` export at `uiSlice.ts:80`). **This path imports zero React, zero backend, zero torch/GPU** — it's the local-runnable core.
> - **Vitest config verified** at `frontend/vite.config.js` `test` block: `globals: true`, `environment: 'jsdom'`, `setupFiles: ['./src/test/setup.js']`, `include: ['src/**/*.test.{js,jsx,ts,tsx}']`, `css: false`. The setup file (`src/test/setup.js`) imports `@testing-library/jest-dom/vitest`, **initializes the real i18n instance** (`import '../i18n'`, `fallbackLng:'en'` → `t()` returns real English, not bare keys), and installs an **in-memory `localStorage` mock** — so persistence-shape and i18n assertions are honest without touching disk.
> - **Component-test idiom verified.** `@testing-library/react` (`render` / `renderHook`) is in devDeps (`@testing-library/react ^16.3.2`, `jsdom`) and already used by 14 tests (e.g. `components/ExportModal.test.jsx`, `store/store.test.js` `renderHook(() => useAppStore(...))`). API/network mocking via `vi.fn()` is established (`components/NetworkToggle.test.jsx:12,19` stubs `global.fetch`). The VoiceGallery handler tests **`vi.mock('../api/archetypes')` / `vi.mock('../api/community')`** — so they never hit the network and never import the backend.
> - **CI gates pinned to `.github/workflows/ci.yml`.** The PR-gated `test` job runs, in order: `uv run pytest tests/` (`:67` — this is where `tests/test_no_hardcoded_cjk.py` runs, so the i18n/CJK acceptance is enforced server-side), `uv run pytest backend/tests/` (`:81`, isolated), `bun run typecheck:ci` (`:103` = `tsc --noEmit --checkJs false` — **only `.ts`/`.tsx` block**, which is exactly what catches a missing `'audiobook'` in the `AppMode` union; the touched `.jsx` files are *not* type-gated), `bunx vitest run` (`:106`), then the legacy node:test runner. The cross-platform `tauri-cross-platform` matrix (`:115-`) `needs: test` but is a Rust `cargo check` — untouched by this pure-frontend task. Nothing previously specified was removed.
