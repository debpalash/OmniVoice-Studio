...
| **Dub-length fitting (audio speedup + video slowdown)** | A– (Smart Fit complete: Phase A planner + generate path + fit fingerprints; Phase B video-retime export with two-tier single-pass/batched pipeline, drift absorption, fitted subtitles) | ❌ | ✅ **`_rate.py` — the crown jewel** | ❌ | Gap closed — Action 1 reimplemented clean-room (`services/fit_planner.py` + `services/video_retime.py`); export-side retime shipped in Phase B |
