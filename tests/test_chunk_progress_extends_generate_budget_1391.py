...
@pytest.mark.asyncio
async def test_chunks_may_be_far_slower_than_a_load_heartbeat(mm, pool):
...
def test_every_multi_part_render_loop_reports_progress():
    ...
    loops = [
        ...
        and any(
            ...
            and c.func.value.id in {"parts", "items"}
            for c in ast.walk(n)
        )
    ]
    assert len(loops) >= 3, f"expected the three multi-part render loops, found {len(loops)}"
    for loop in loops:
