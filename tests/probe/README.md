...
| L3 Desktop | _(next)_ | planned | Thin smoke: bundle launches · `invoke()`→backend reachable · window renders. (Tauri macOS has no official WebDriver — we substitute backend-over-HTTP + browser, per the architecture decision.) |
