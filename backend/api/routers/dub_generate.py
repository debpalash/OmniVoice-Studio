...
async def dub_generate(job_id: str, req: DubRequest):
    ...
    async def _stream(task_id):
        ...
        def _release_audio_tensors(*objs) -> None:
            """Best-effort RAM/VRAM cleanup after a segment is safely on disk."""
            for obj in objs:
                try:
                    del obj
                except Exception:
        ...
        def _write_memmap_wav_atomic(target_path: str, samples, sample_rate: int) -> None:
            """Watermark and write a mono float32 memmap without loading it all."""
            import tempfile
            import wave
            import numpy as np
            ...
            try:
                with wave.open(tmp_path, "wb") as wf:
                    ...
                    for off in range(0, total_len, chunk_samples):
                        ...
                        chunk_tensor = embed_watermark(chunk_tensor, sample_rate)
                        chunk = (
                            chunk_tensor
                            .detach()
                            .cpu()
        ...
        for i, seg in enumerate(req.segments):
            ...
            try:
                ...
                sr = _model.sampling_rate
                all_segment_wavs.append(_store_mix_wav(seg.start, seg.end, torch.zeros(1, int(seg_duration * sr)), sr, f"mix_{seg_id}"))
                sync_scores.append(1.0)
