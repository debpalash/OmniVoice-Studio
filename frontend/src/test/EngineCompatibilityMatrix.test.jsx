...
describe("EngineCompatibilityMatrix", () => {
  ...
  it("formats disk sizes with the active locale", () => {
    expect(fmtDiskBytes(1.5 * 1024 ** 3, "unknown", "de-DE")).toMatch(
      /^1,50\sGB$/u,
    );
  });
  ...
  it("lists available engines first, keeping registration order inside each group", async () => {
    ...
    const order = Array.from(
      document.querySelectorAll(".engine-matrix__name"),
  ...
  it("dims an unavailable engine's name without fading its evidence", async () => {
    ...
    expect(unavailable.closest(".engine-matrix__row").className).not.toContain(
      "opacity-",
    );
  });
  ...
  it("renders one row per backend with the documented columns", async () => {
    ...
    ).toEqual(["Engine", "Status", "GPU compat", "Isolation", "Actions"]);
  });
  ...
  it("shows separate estimates and measured disk categories on demand", async () => {
    ...
    await waitFor(() =>
      expect(apiGetDiskUsage).toHaveBeenCalledWith("omnivoice"),
    );
    ...
    expect(
      within(details).getByText("Estimate confidence"),
    ).toBeInTheDocument();
    expect(within(details).getByText("Estimated")).toBeInTheDocument();
    expect(
      within(details).getByText("Measurement confidence"),
    ).toBeInTheDocument();
  ...
  it("shows isolation_mode badge per row (subprocess for IndexTTS, in-process for the others)", async () => {
    ...
    const indexRow = screen
      .getByText("IndexTTS2 (test)")
      .closest('[role="row"]');
    const omniRow = screen
      .getByText("OmniVoice (test)")
      .closest('[role="row"]');
    const kittenRow = screen
      .getByText("KittenTTS (test)")
      .closest('[role="row"]');
  ...
  it("shows the install reason in the expansion panel when a backend is unavailable", async () => {
    ...
    const badge = within(kittenRow).getByText(
      (_, el) =>
        el?.tagName === "SPAN" &&
        /^\s*Unavailable\s*$/.test(el.textContent || ""),
    );
  ...
  it("clicking Test engine fires getEngineHealth and renders latency_ms", async () => {
    ...
    await waitFor(() => {
      expect(
        within(indexRow).getByTestId("health-result-indextts2"),
      ).toBeInTheDocument();
    });
  ...
  it('highlights the effective device chip + shows an "accelerated" badge', async () => {
    ...
    expect(
      within(row).getByText("CUDA").classList.contains("is-effective"),
    ).toBe(true);
    // a non-effective chip does not
    expect(
      within(row).getByText("MPS").classList.contains("is-effective"),
    ).toBe(false);
  });
  ...
  it("renders a failure marker when the health route returns ok=false", async () => {
    ...
    const indexRow = screen
      ...
      .closest('[role="row"]');
    fireEvent.click(
      within(indexRow).getByRole("button", { name: /test indextts2/i }),
    );
  ...
  it("surfaces the routing reason as visible text, not only a hover title", async () => {
    ...
    expect(
      within(row).getByTestId("routing-reason-fallback"),
    ).toHaveTextContent("engine has no CUDA path; running on CPU");
    // A clean accelerated row (no caveat reason) shows no reason line.
    const accelRow = screen.getByText("Accel TTS").closest('[role="row"]');
    expect(
      within(accelRow).queryByTestId("routing-reason-accel"),
    ).not.toBeInTheDocument();
  });
  ...
  it('labels an in-process health check "deps OK" while subprocess shows real ms', async () => {
    ...
    fireEvent.click(
      within(omniRow).getByRole("button", { name: "Test OmniVoice (test)" }),
    );
    await waitFor(() => {
      expect(
        within(omniRow).getByTestId("health-result-omnivoice"),
      ).toHaveTextContent("deps OK");
    });
  ...
  it("reflects the new active engine after select resolves, without a manual Refresh", async () => {
    ...
    await waitFor(() => screen.getByText("IndexTTS2 (test)"));
    const indexRow = () =>
      screen.getByText("IndexTTS2 (test)").closest('[role="row"]');
    // Not active yet.
    expect(within(indexRow()).queryByText("active")).not.toBeInTheDocument();

    fireEvent.click(
      within(indexRow()).getByRole("button", { name: /use indextts2/i }),
    );
    await waitFor(() =>
      expect(onSelect).toHaveBeenCalledWith("tts", "indextts2"),
    );
  ...
  it('mounts the Supertonic license dialog when "Accept license" is clicked', async () => {
        ...
        backends: [
          {
            ...
            available: false,
            reason:
              "Supertonic-3 license not accepted — review and accept to enable it.",
            install_hint: null,
            last_error: null,
            isolation_mode: "in-process",
            gpu_compat: ["cpu"],
    ...
    expect(
      screen.queryByText("Supertonic-3 — License Acceptance"),
    ).not.toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Supertonic-3 — License Acceptance"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("mounts the PocketTTS terms dialog from an unavailable engine row", async () => {
        ...
        backends: [
          {
            ...
            available: false,
            reason:
              "PocketTTS license not accepted. Review the terms to enable it.",
            install_hint: null,
            last_error: null,
            isolation_mode: "subprocess",
            gpu_compat: ["cpu"],
    ...
    await waitFor(() => {
      expect(
        screen.getByText("PocketTTS License Acceptance"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("Review the access conditions"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("clicking Self-test runs a real synthesis and renders audio seconds + sample rate", async () => {
    ...
    const omniRow = screen
      ...
      .closest('[role="row"]');
    fireEvent.click(
      within(omniRow).getByRole("button", { name: /self-test omnivoice/i }),
    );

    await waitFor(() =>
      expect(apiSelfTestEngine).toHaveBeenCalledWith("omnivoice"),
    );
    await waitFor(() => {
      expect(
        within(omniRow).getByTestId("selftest-result-omnivoice"),
      ).toHaveTextContent("0.82s @ 24 kHz in 820 ms");
    });
  });
  ...
  it("renders a timed-out marker when the self-test outruns the timeout", async () => {
    ...
    await waitFor(() => {
      expect(
        within(omniRow).getByTestId("selftest-result-omnivoice"),
      ).toHaveTextContent("Self-test timed out");
    });
  });
  ...
  it("does not offer Self-test for a subprocess engine (spawn-and-ping only)", async () => {
    ...
    expect(
      within(indexRow).queryByRole("button", { name: /self-test/i }),
    ).not.toBeInTheDocument();
  });
  ...
  it("does not offer Self-test on a non-TTS family (ASR)", async () => {
    ...
    expect(
      within(row).queryByRole("button", { name: /self-test/i }),
    ).not.toBeInTheDocument();
  });
  ...
  it("shows no setup snippet for a bundled engine", async () => {
    ...
    fireEvent.click(screen.getByTestId("why-toggle-kittentts"));
    expect(
      screen.queryByTestId("setup-snippet-kittentts"),
    ).not.toBeInTheDocument();
  });
  ...
  it("renders the curated-model picker for mlx-audio, pre-selected to the active model", async () => {
    ...
    expect(
      within(select).getByRole("option", { name: "CSM (voice cloning)" }),
    ).toBeInTheDocument();
    expect(
      within(select).getByRole("option", { name: "OuteTTS" }),
    ).toBeInTheDocument();
  });
  ...
  it("picking a different curated model calls onSelect with the model key and refreshes", async () => {
    ...
    const apiListEngines = vi.fn(async () =>
      mlxAudioResponse({ activeModelId }),
    );
    ...
    await waitFor(() => {
      expect(screen.getByTestId("curated-model-select-mlx-audio")).toHaveValue(
        "csm",
      );
    });
    expect(apiListEngines.mock.calls.length).toBeGreaterThanOrEqual(2);
  ...
  it("does not render a curated-model picker for engines without curated_models", async () => {
    ...
    await waitFor(() => screen.getByText("OmniVoice (test)"));
    expect(
      screen.queryByTestId(/curated-model-select-/),
    ).not.toBeInTheDocument();
  });
  ...
  it("keeps the family switcher by default (standalone mounts unchanged)", async () => {
    ...
    expect(document.querySelectorAll(".engine-matrix__tab-family").length).toBe(
      3,
    );
    ...
    expect(
      document.querySelector(".engine-matrix__tab-active"),
  ...
  it("names what each family does in pinned mode (one description line)", async () => {
    ...
    await waitFor(() => screen.getByText("WhisperX (test)"));
    expect(screen.getByTestId("family-desc-asr")).toHaveTextContent(
      /turns audio into text/i,
    );
  });
  ...
  it("renders a deterministic identity mark on every engine row", async () => {
    ...
    expect(screen.getByTestId("engine-mark-omnivoice")).toHaveAttribute(
      "aria-hidden",
      "true",
    );
  });
  ...
  it("renders the ok-with-advice hint as a quiet inline line on an available row", async () => {
    ...
    expect(
      screen.queryByTestId("engine-hint-indextts2"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("engine-hint-kittentts"),
    ).not.toBeInTheDocument();
  });
  ...
  it("badges voice-cloning-capable engines — and only on explicit true", async () => {
    ...
    await waitFor(() => screen.getByText("OmniVoice (test)"));
    expect(screen.getByTestId("clone-badge-omnivoice")).toHaveTextContent(
      "Voice cloning",
    );
    expect(
      screen.queryByTestId("clone-badge-indextts2"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("clone-badge-kittentts"),
    ).not.toBeInTheDocument();
  });
  ...
  it("never badges cloning on a non-TTS family (capability is TTS-only)", async () => {
    ...
    await waitFor(() => screen.getByText("WhisperX (test)"));
    expect(
      screen.queryByTestId("clone-badge-whisperx"),
    ).not.toBeInTheDocument();
  });
  ...
  it('marks a loaded engine "In memory" and unloads it via its /model/loaded id', async () => {
    ...
    const row = () =>
      screen.getByText("IndexTTS2 (test)").closest('[role="row"]');
    await waitFor(() => {
      expect(within(row()).getByTestId("resident-indextts2")).toHaveTextContent(
        "In memory",
      );
    });
    ...
    const omniRow = screen
      ...
      .closest('[role="row"]');
    expect(
      within(omniRow).queryByTestId("resident-omnivoice"),
    ).not.toBeInTheDocument();
    expect(
    ...
    fireEvent.click(
      within(row()).getByRole("button", { name: /unload indextts2/i }),
    );
    await waitFor(() => {
    ...
    await waitFor(() => {
      expect(
        within(row()).queryByTestId("resident-indextts2"),
      ).not.toBeInTheDocument();
    });
    expect(
  ...
  it("renders strict two-line rows: fixed height, truncated name with full title", async () => {
    ...
    for (const row of document.querySelectorAll("[data-engine-id]")) {
      ...
      expect(row.querySelector(".engine-matrix__id")).toHaveClass(
  ...
  it("uses the same wider grid tracks for Catalogue headers and rows", async () => {
    ...
    for (const row of document.querySelectorAll("[data-engine-id]")) {
      ...
      expect(row.className).toContain(
        "@max-[1230px]/catalogue-shell:grid-cols-",
      );
    }
  });
  ...
  it("opens and closes the details expansion panel below the row", async () => {
    ...
    expect(
      screen.queryByTestId("engine-detail-kittentts"),
    ).not.toBeInTheDocument();
  ...
  it("available rows offer no details toggle (hints stay inline on line 2)", async () => {
    ...
    await waitFor(() => screen.getByText("OmniVoice (test)"));
    expect(
      screen.queryByTestId("why-toggle-omnivoice"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("why-toggle-indextts2"),
    ).not.toBeInTheDocument();
  ...
  function makeInstallStatus(jobState, overrides = {}) {
    return {
      ...
      job: {
        ...
        log: [
          "Cloning https://github.com/index-tts/index-tts.git (git, depth 1) …",
        ],
        error: null,
        remediation: null,
  ...
  it("installable unavailable rows get an Install button; clicking it starts the job and shows step progress", async () => {
    ...
    const apiInstallEngine = vi
      .fn()
      .mockResolvedValue({ status: "started", engine: "indextts2" });
    ...
    await waitFor(() =>
      expect(apiInstallEngine).toHaveBeenCalledWith("indextts2"),
    );
    ...
    expect(
      within(progress).getByText(/Checking uv and disk space/),
    ).toBeInTheDocument();
    const running = progress.querySelector(
      '[data-install-step="fetch_source"]',
    );
    expect(running).toHaveAttribute("data-step-state", "running");
    // The live log tail is visible while the job runs.
    expect(
      within(progress).getByText(/Cloning https:\/\//),
    ).toBeInTheDocument();
    // The button reflects the in-flight job.
    expect(screen.getByTestId("install-indextts2")).toHaveTextContent(
      "Installing…",
    );
  });
  ...
  it("a failed job renders the error with its remediation and offers Retry", async () => {
    ...
    expect(
      within(progress).getByText(
        /Not enough disk space .* Free up disk space and retry\./,
      ),
    ).toBeInTheDocument();
    expect(screen.getByTestId("install-indextts2")).toHaveTextContent(
      "Retry install",
  ...
  it("a wedged probe cannot stall the Install click forever, nor clobber it late", async () => {
    ...
    expect(
      screen.getByTestId("install-progress-indextts2"),
    ).toBeInTheDocument();
  });
  ...
  it("already_installed responses skip the job and just reload the matrix", async () => {
    ...
    expect(
      screen.queryByTestId("install-progress-indextts2"),
    ).not.toBeInTheDocument();
  });
  ...
  it("demotes the manual setup snippet to a collapsed fallback on installable rows", async () => {
    ...
    expect(
      within(manual).getByTestId("setup-snippet-indextts2"),
    ).toBeInTheDocument();
  });
  ...
  it("keeps the setup snippet top-level on rows without a one-click installer", async () => {
    ...
    expect(
      screen.queryByTestId("manual-install-kittentts"),
    ).not.toBeInTheDocument();
