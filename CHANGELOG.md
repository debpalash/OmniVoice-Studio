...
### Fixed
- The OpenAI-compatible `/v1/audio/speech` route now reuses the shared cached engine instance for explicit `model` ids instead of constructing a fresh engine — and its sidecar/model load — on every request (~28s floor per call for subprocess engines), and hands back any other resident engine's model before generating, the same single-engine-resident discipline `/generate` applies (#1614) — thanks @paoloantinori!
