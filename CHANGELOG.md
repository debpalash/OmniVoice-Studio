...
### Added
- One-command install on every desktop OS: `curl -fsSL https://voicestudio.sh/install | sh` (macOS/Linux/WSL) and `irm https://voicestudio.sh/install | iex` (Windows) — the URL serves the right script per platform via a Cloudflare Worker, and the new `scripts/install.ps1` gives Windows a source install path (winget deps + uv + bun) that previously required hand-following the docs. A 3-OS install-smoke workflow runs both scripts end-to-end when they change (#1626)
