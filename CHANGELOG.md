...
- A UI whose API requests land on the wrong host — a rehosted frontend, or a reverse proxy with no API route — no longer echoes that host's raw 404 page as the error. It now says the responding server is not an VoiceStudio backend and points at the Backend URL setting and the proxy route. (#1385)
