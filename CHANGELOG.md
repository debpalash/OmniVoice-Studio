...
- **Settings → Usage: see what you've made, counted entirely on your own machine.** A cloud-analytics integration (PostHog) was proposed and **rejected** — sending usage events to a third-party endpoint would break the one promise this app is built on: *nothing leaves your machine*. So the same question gets a local answer instead. The new Usage panel shows takes generated, audio produced, voices, days used, and a breakdown by mode and language — all computed from the history already stored in your own database. It collects nothing new, stores nothing new, and transmits nothing anywhere; the panel says so plainly, in the app, where you can see it. There is no analytics service, no token, and no opt-out to hunt for, because there is nothing to opt out of.
