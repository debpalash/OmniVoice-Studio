...
- A machine with a GPU that ends up on CPU now says why — a missing device node, a permissions problem, or a card newer than the installed ROCm each read differently. Before, it looked identical to having no GPU at all. (#1274, #1228)
