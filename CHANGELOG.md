...
- **FunASR/SenseVoice transcription no longer crashes when inline speaker diarization is enabled.** The pinned FunASR 1.3.1 release defaults CAM++ to a punctuation-dependent segmentation mode, while OmniVoice intentionally does not load a second punctuation model — so the first transcription failed inside FunASR before returning any text. The backend now uses VAD speaker segments, requests SenseVoice timestamps, and accepts the release's `sentence` output field so dubbing receives timed speaker turns as intended. (#182)
